...
def test_sanity_files_specific_file(monkeypatch):
    ...
    monkeypatch.setattr(cmd_repo, "_require_api", lambda _cm: (0, object()))
    monkeypatch.setattr(
        cmd_repo,
        "process_fits_file",
    ...
    ret = cmd_repo.sanity_files(cm, _base_args(file=r"c:\repo\one.fits", show_header=True))
    assert ret == 0
    assert len(calls) == 1
...
def test_sanity_files_list_file(monkeypatch):
    ...
    ret = cmd_repo.sanity_files(cm, _base_args(list=r"c:\tmp\fits_list.txt", dry_run=True))
    assert ret == 0
    assert len(calls) == 1
...
def test_sanity_files_specific_directory(monkeypatch):
    ...
    ret = cmd_repo.sanity_files(cm, _base_args(dir=r"c:\astro\night1", show_header=True))
    assert ret == 0
    assert len(calls) == 1
...
def test_sanity_files_defaults_to_all_configured_volumes(monkeypatch):
    ...
    monkeypatch.setattr(cmd_repo, "_require_api", lambda _cm: (0, object()))
    monkeypatch.setattr(cmd_repo, "_monitor_volumes_from_config", lambda _cm: volumes)
    monkeypatch.setattr(
        cmd_repo,
        "process_fits_dir",
    ...
    ret = cmd_repo.sanity_files(cm, _base_args())
    assert ret == 0
