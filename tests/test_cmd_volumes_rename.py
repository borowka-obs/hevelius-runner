...
def test_rename_single_file(tmp_path, capsys):
    ...
    assert "Caldwell 75" in out
    assert "RCW 38" in out
    assert "1 renamed" in out


def test_rename_dir_skips_unchanged_and_renames_matches(tmp_path, capsys):
    cm = _DummyConfigManager()
...
def test_rename_fails_when_target_exists(tmp_path, capsys):
    cm = _DummyConfigManager()
