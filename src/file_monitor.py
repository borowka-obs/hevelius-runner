...
from image_formats import SUPPORTED_EXTENSIONS, is_supported_image
...
class ImageFileHandler(FileSystemEventHandler):
    def __init__(
        ...
        Args:
            callback: Function to call when a new supported image file is detected
            file_extensions: Extensions to monitor (default: all supported formats)
        """
        self.callback = callback
        if file_extensions is None:
            self.file_extensions = frozenset(SUPPORTED_EXTENSIONS)
        else:
            self.file_extensions = frozenset(
    ...
    def _handle_new_file(self, file_path: str):
        ...
        with self._processing_lock:
            ...
            try:
                self.logger.info(f"Processing new image file: {file_path}")
...
class FileMonitor:
    ...
    def _parse_monitor_volumes(self, config: Dict[str, Any]) -> List[Tuple[Path, str]]:
        ...
        raise ValueError(
            "No image monitor directories configured. Set paths.volumes in your config."
        )

    def start(self, callback: Callable[[str], None]):
        """
        Start monitoring for new image files.
    ...
    def process_existing_files(self):
        """Process any existing supported image files in all monitor volumes."""
        try:
            for volume_path, nickname in self.monitor_volumes:
                for file_path in volume_path.iterdir():
